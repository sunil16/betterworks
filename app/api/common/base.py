...
class BaseResource(object):
    ...
    # }
    def show_log(self: any, req: falcon.Request, description: str = '') -> str:
        pass
        # print('--- {type(self).__module__}.{type(self).__name__} ---- service called with parms: {req.context} {description}\n')
    ...
    def on_get(self, req, res):
        if req.path == "/":
            res.status = falcon.HTTP_200
            res.body = self.to_json({'message': 'hello services'})
        else:
